...
class TestInlineSystemMessageInMessagesArray:
    ...
    Mid-conversation turns stay in place for prefix caching. A trailing system
    suffix is folded into the leading block so current Claude Code requests do
    not leave generation positioned after a system turn.
    """
    ...
    def test_inline_system_merged_with_top_level_system(self):
        ...
        # Top-level and trailing inline system text are merged; the billing
        # attribution block is stripped.
    ...
    def test_inline_system_billing_header_mixed_with_content(self):
        ...
        # Billing header stripped; remaining trailing system content is hoisted.
        assert len(result.messages) == 2
        assert result.messages[0]["role"] == "system"
