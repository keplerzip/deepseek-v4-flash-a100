...
class FakeHfTokenizer:
    ...
    def __len__(self) -> int:
        # DeepSeek's added-token IDs are already inside config.vocab_size.
...
def test_deepseek_v4_length_does_not_double_count_added_vocab():
    tokenizer = _tokenizer()

    assert tokenizer.get_added_vocab() == {"</think>": 100}
    assert len(tokenizer) == tokenizer.vocab_size == 100
