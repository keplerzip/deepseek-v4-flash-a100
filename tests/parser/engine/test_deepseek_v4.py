...
from vllm.parser.deepseek_v4 import (
    ...
    DSML_INVOKE_PREFIX,
    DSML_MALFORMED_TOOL_END,
    DSML_MALFORMED_TOOL_START,
    DSML_THINK_END,
    DSML_THINK_START,
    DSML_TOOL_END,
    DSML_TOOL_START,
...
class TestOrphanInvokeNameValidation:
    ...
    Wrapped and recovered calls are both rejected when their names were not
    declared; recovered calls additionally stay provisional through the full
    invoke closer so a truncated candidate can be rolled back losslessly.
    """
    ...
    def test_wrapped_undeclared_name_is_not_executable(
        self, mock_tokenizer, mock_request, weather_tool
    ):
        """A native wrapper cannot bypass the request's declared tool set."""
        parser = self._declared_parser(mock_tokenizer, mock_request, weather_tool)
    ...
    def test_valid_name_without_invoke_end_rolls_back(
        self, mock_tokenizer, mock_request, weather_tool
    ):
        parser = self._declared_parser(mock_tokenizer, mock_request, weather_tool)
        text = (
    ...
    def test_tool_end_without_invoke_end_rolls_back(
        self, mock_tokenizer, mock_request, weather_tool
    ):
        parser = self._declared_parser(mock_tokenizer, mock_request, weather_tool)
        text = (
    ...
    def test_truncated_recovery_drops_eos_special_token(
        self, mock_request, weather_tool
    ):
        ...
        parser = DeepSeekV4Parser(tokenizer, tools=[weather_tool])
        text = (
            DSML_INVOKE_PREFIX
            + "get_weather"
            + DSML_INVOKE_NAME_END
    ...
    def test_streaming_emits_nothing_before_invoke_end(
        self, mock_tokenizer, mock_request, weather_tool
    ):
        parser = self._declared_parser(mock_tokenizer, mock_request, weather_tool)
        chunks = [
        ...
        assert finish_delta is not None
        assert not finish_delta.tool_calls
        assert (collect_content(results) + (finish_delta.content or "")) == "".join(
            chunks
        )

    def test_parallel_second_undeclared_name_never_executes(
        self, mock_tokenizer, mock_request, weather_tool
    ):
        parser = self._declared_parser(mock_tokenizer, mock_request, weather_tool)
        first = _invoke("get_weather", ("location", "true", "NYC"))
        second = _invoke("delete_everything", ("path", "true", "/"))
        result = parser.extract_tool_calls(first + "\n" + second, mock_request)

        assert result.tools_called is True
        assert [tc.function.name for tc in result.tool_calls] == ["get_weather"]
        assert "delete_everything" in (result.content or "")

    def test_known_malformed_wrapper_recovers_without_dsml_leak(
        self, mock_tokenizer, mock_request, weather_tool
    ):
        parser = self._declared_parser(mock_tokenizer, mock_request, weather_tool)
        text = (
...
class TestDelegatingParserLargeDelta:
    ...
    def test_eos_drop_token_does_not_swallow_tool_calls(self):
        ...
        request = _test_request(tools=_DSV4_WEATHER_TOOLS)
