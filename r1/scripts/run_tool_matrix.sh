...
# Live target-only matrix: stream/non-stream, auto/required/none, thinking
# on/off, and synchronized concurrency 1/4/8.  Python runs in the R1 image.
...
run_dir="$RESULT_DIR/tool-matrix-$timestamp"
mkdir -p "$run_dir"
host_user="$(id -u):$(id -g)"
...
docker_cmd run --rm --network host \
  ...
  /audit/tests/tool_matrix_test.py \
