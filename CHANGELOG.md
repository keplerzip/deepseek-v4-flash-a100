...
- 目标机方案二 DSpark greedy k=7 已确认完整正确运行；实测 decode TPS
  典型为 `130–180 tokens/s`，现场最高观测值为 `355 tokens/s`（非持续或聚合吞吐），
  完整 1M 请求 KV 容量并发为 `6.95x`，Codex Responses 正常。
...
- 新增以精确 R2 镜像 ID 为基础的断网增量交付；目标机只构建文件覆盖层，不下载、
  不安装依赖、不重新编译 CUDA，并在启动前复验结果文件和包元数据。
