...
class StreamingParserEngine:
    ...
    def finish(self) -> list[SemanticEvent]:
        ...
        if self._hold_active:
            # A recovery is provisional until the complete invoke closer.
            # Restore the exact raw candidate when the stream ends earlier.
        ...
        if self.state == ParserState.TOOL_BETWEEN and self._recovered_tool_call:
            # A complete provisional call already emitted TOOL_CALL_END.
            # Drop whitespace still held as inter-invoke padding and avoid a
            # duplicate end event when the malformed outer wrapper is absent.
            self._pending_between_text = ""
            self._recovered_tool_call = False
    ...
    def _on_terminal(self, terminal: str, value: str) -> list[SemanticEvent]:
        ...
        if transition is None:
            # Special tokens configured for automatic dropping must never be
            # copied into provisional raw buffers (c848ab5 safety invariant).
    ...
    def _emit_for_state(self, text: str) -> list[SemanticEvent]:
        if self._hold_active:
            ...
            if self.state == ParserState.TOOL_ARGS:
                self._held_raw.append(text)
                self._held_events.extend(self._emit_for_state_now(text))
                return []
            return self._abort_hold("".join(self._held_raw) + text)
    ...
    def _apply_transition(
        ...
        if self._hold_active:
            return self._advance_hold(transition, value)
        if transition.validate_tool_name:
            # TOOL_BETWEEN is shared by native wrapped parallel calls and
            # recovered bare calls.  Only the latter require another
            # provisional hold; a native wrapper already establishes the
            # lexical boundary and is validated by ParserEngine globally.
            if self.state == ParserState.TOOL_BETWEEN and not self._recovered_tool_call:
    ...
    def _advance_hold(
        ...
        """Advance a provisional recovery, committing only at invoke end."""
