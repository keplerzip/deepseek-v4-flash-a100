...
# Known malformed outer wrapper emitted at long context (missing underscore).
# It is consumed so raw DSML never reaches clients; the inner invoke still
# follows the declared-name, full-closer provisional recovery path.
DSML_MALFORMED_TOOL_START = f"<{_DSML}toolcalls>"
DSML_MALFORMED_TOOL_END = f"</{_DSML}toolcalls>"
...
def deepseek_v4_config(thinking: bool = False) -> ParserEngineConfig:
    return ParserEngineConfig(
        ...
        terminals={
            ...
            "MALFORMED_TOOL_START": DSML_MALFORMED_TOOL_START,
            "MALFORMED_TOOL_END": DSML_MALFORMED_TOOL_END,
        ...
        transitions={
            ...
            # Drop only the known malformed wrapper.  Its inner invoke is
            # independently validated and remains provisional until </invoke>.
            (ParserState.CONTENT, "MALFORMED_TOOL_START"): Transition(
                ParserState.CONTENT,
                (),
            ),
            (ParserState.REASONING, "MALFORMED_TOOL_START"): Transition(
                ParserState.REASONING,
                (),
            ),
            (ParserState.CONTENT, "MALFORMED_TOOL_END"): Transition(
                ParserState.CONTENT,
                (),
            ),
            ...
            (ParserState.TOOL_BETWEEN, "MALFORMED_TOOL_END"): Transition(
                ParserState.CONTENT,
                (),
            ),
        },
