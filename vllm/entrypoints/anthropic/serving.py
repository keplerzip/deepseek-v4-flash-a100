...
class AnthropicServingMessages(OpenAIServingChat):
    ...
    def _convert_system_message(
        ...
        *,
        ...
        # Fold all inline system turns when the template requires system-first
        # ordering.  Otherwise fold only a trailing suffix: Claude Code
        # >=2.1.207 sends [user, system] for its agent registry, and leaving
        # generation positioned after that system turn derails tool calling.
        # Mid-conversation system turns remain in place for prefix caching.
    ...
    def _trailing_system_messages(messages: list) -> list:
        """Return the suffix of ``messages`` composed only of system turns."""
        start = len(messages)
